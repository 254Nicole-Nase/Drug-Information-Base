...
CREATE TABLE public.drug_labels (
  id text PRIMARY KEY,
  set_id text,
  effective_time text,
  brand_name text,
...
CREATE TABLE public.label_chunks (
...
CREATE INDEX idx_label_chunks_embedding ON public.label_chunks USING ivfflat (embedding vector_cosine_ops);
CREATE INDEX idx_label_chunks_tsv ON public.label_chunks USING gin (search_tsv);
...
CREATE POLICY "Anyone can read drug labels" ON public.drug_labels FOR SELECT TO anon, authenticated USING (true);
CREATE POLICY "Anyone can read label chunks" ON public.label_chunks FOR SELECT TO anon, authenticated USING (true);
